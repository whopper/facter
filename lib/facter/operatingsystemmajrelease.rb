...
#   osfamily RedHat derivatives and Debian. Uses operatingsystemrelease
#   key to the first non decimal character for operatingsystem Solaris
#
...
Facter.add(:operatingsystemmajrelease) do
  confine do
    !Facter.value("os")["releasemajor"].nil?
  end

  setcode do
    if (releasemajor = Facter.value("os")["releasemajor"])
      releasemajor
    end
  end
end

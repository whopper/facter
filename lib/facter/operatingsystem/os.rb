    ...
    class GNU < Base
      ...
      def get_lsb_facts_hash
        ...
        if lsbdistcodename = get_lsbdistcodename
          lsb_hash["lsbdistcodename"] = lsbdistcodename
        end

        if lsbdistid = get_lsbdistid
          lsb_hash["lsbdistid"] = lsbdistid
        end

        if lsbdistdescription = get_lsbdistdescription
          lsb_hash["lsbdistdescription"] = lsbdistdescription
        end

        if lsbrelease = get_lsbrelease
          lsb_hash["lsbrelease"] = lsbrelease
        end

        if lsbdistrelease = get_lsbdistrelease
          lsb_hash["lsbdistrelease"] = lsbdistrelease
        end

        if lsbmajdistrelease = get_lsbmajdistrelease
          lsb_hash["lsbmajdistrelease"]  = lsbmajdistrelease
        end
        lsb_hash
      end
    end

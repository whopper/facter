...
Facter.add(:os, :type => :aggregate) do
  ...
  chunk(:release) do
    os_hash = {}
    os_hash["release"] = {}
    if (release = os.get_operatingsystemrelease)
      os_hash["release"] = release
      os_hash
    end
  end

  chunk(:releasemajor) do
    os_hash = {}
    if (releasemajor = os.get_operatingsystemmajrelease)
      os_hash["releasemajor"] = releasemajor
      os_hash
    end
  end
